...
- (NSData*) getPostData {
	...
	{
		return [[HTTPRequest buildQueryStringFromDictionary:postParameters] dataUsingEncoding:NSUTF8StringEncoding];
	} else if (postData) {
...
+ (NSString*)buildQueryStringFromDictionary:(NSDictionary*)_dictionary {
	...
	for (int i = 0; i < [keys count]; i++) {
		...
		[_postData appendFormat:@"%@=%@", key, [[[[_dictionary valueForKey:key] stringByAddingPercentEscapesUsingEncoding:NSASCIIStringEncoding] stringByReplacingOccurrencesOfString:@"&" withString:@"%26"] stringByReplacingOccurrencesOfString:@"+" withString:@"%2B"]];
		if (i < [keys count] - 1)
			[_postData appendString:@"&"];
	}
	return [NSString stringWithString:_postData];
}
